        ...
        <main>
            <div class="span12 text_align">
                ...
                <#list posts as post>
                    <div class="span" style="width:50rem;border:0.5rem solid black;font-size:2rem">
                        <p>
                            <a href="<#if post.secretEnabled == true>${settings.getSetting('admin.url')}<#else>${settings.getSetting('post.url')}</#if>${post.path}">${post.title} <#if post.secretEnabled == true>(secret)</#if></a>
                        </p>
                    </div>
                </#list>
            </div>
